...
**Keep prints a surprise:** Admin shows a warning banner only when there are failed prints, with retry buttons and concealed contents. Failures refresh when you enter Admin, return to the browser page or use **refresh**; a successful retry removes the failure, and the banner disappears when none remain. The closed **print troubleshooting** section fetches a metadata-only log when opened; senders, text and images require **reveal content** on an individual print. Scheduled, queued and cancelled prints are excluded until you choose **include undelivered prints** and confirm. Closing the log, switching console tabs or leaving the browser page clears revealed contents and resets that opt-in. This is friction against accidental spoilers, not a new permission boundary: the authenticated owner retains troubleshooting access.
